...
export const LetterPage: React.FC = () => (
  ...
  {/* Decorative Hearts - smaller and in corners */}
  <HeartIcon className="absolute top-2 left-2 w-10 h-10 animate-heart-glow" />
  <HeartIcon className="absolute top-2 right-2 w-10 h-10 animate-heart-glow delay-200" />
  <HeartIcon className="absolute bottom-2 left-2 w-10 h-10 animate-heart-glow delay-400" />
  <HeartIcon className="absolute bottom-2 right-2 w-10 h-10 animate-heart-glow delay-600" />
  {/* Decorative Swirls - smaller and in corners */}
  <SwirlIcon className="absolute top-0 left-0 w-16 h-8 opacity-40" />
  <SwirlIcon className="absolute bottom-0 right-0 w-16 h-8 opacity-40" />
    {/* Letter Content */}
    <div className="flex flex-col items-center justify-center px-8 py-12">
      ...
      <div className="font-pacifico text-3xl text-coral">Aayushi</div>
    </div>
    <style>{`
      .animate-heart-glow {
      ...
      @keyframes heart-glow {
        0% { filter: drop-shadow(0 0 0px #FF6B6B); opacity: 0.8; }
        50% { filter: drop-shadow(0 0 12px #FF6B6B); opacity: 1; }
        100% { filter: drop-shadow(0 0 0px #FF6B6B); opacity: 0.8; }
      }
    `}</style>
  </div>
);
